...
enum TranscriptFormatter {
    ...
    /// Self-contained Markdown block for just the summary + action items,
    /// titled with the meeting name so the paste is useful on its own.
    /// Returns nil if the transcript has no summary yet.
    static func renderSummaryMarkdown(_ doc: TranscriptDocument) -> String? {
        let hasSummary = (doc.summary?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false)
        let hasActions = (doc.actionItems?.isEmpty == false)
        guard hasSummary || hasActions else { return nil }
